import { MailDrivers } from '@ioc:Adonis/Addons/Mail';
/**
 * Expected shape of the config accepted by the "mailConfig"
 * method
 */
declare type MailConfig = {
    mailers: {
        [name: string]: {
            [K in keyof MailDrivers]: MailDrivers[K]['config'] & {
                driver: K;
            };
        }[keyof MailDrivers];
    };
};
/**
 * Define config for mail
 */
export declare function mailConfig<T extends MailConfig & {
    mailer: keyof T['mailers'];
}>(config: T): T;
/**
 * Pull mailers from the config defined inside the "config/mail.ts"
 * file
 */
export declare type InferMailersFromConfig<T extends MailConfig> = {
    [K in keyof T['mailers']]: MailDrivers[T['mailers'][K]['driver']];
};
export {};
